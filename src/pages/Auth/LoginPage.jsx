...
export default function LoginPage() {
  ...
  const onSubmit = async (e) => {
    ...
    setLocalError("");
    try {
      // Transform formData to match server expected format (use lowercase keys)
      const loginData = {
      ...
      // Friendly fallback for 401 with no body
      let msg = serverMsg || error?.message || "Login failed";
      if (error?.response?.status === 401 && !serverMsg) {
