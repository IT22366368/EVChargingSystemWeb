  ...
  const fetchStation = async () => {
    try {
      ...
      setForm({
        ...
        address: s.address || "",
        city: s.city || "",
        stateProvince: s.stateProvince || s.state || s.province || "",
        ...
        latitude:
          (s.latitude != null && s.latitude !== "")
            ? s.latitude
            : (s.location && s.location.latitude != null ? s.location.latitude : ""),
        longitude:
          (s.longitude != null && s.longitude !== "")
            ? s.longitude
            : (s.location && s.location.longitude != null ? s.location.longitude : ""),
      });
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#1a2955]">
      ...
      <main className="pt-16 pb-8 px-6">
        <div className="max-w-3xl mx-auto text-white">
          <h1 className="text-3xl font-bold mb-4">Edit Charging Station</h1>
          {error && (
          ...
          <form onSubmit={handleSubmit} className="space-y-4 bg-gray-800 p-6 rounded-lg border border-gray-700">
            ...
            <div className="flex gap-3">
              <button type="submit" disabled={loading} className="bg-[#ff7600] px-4 py-2 rounded text-white">{loading ? "Updating..." : "Update Station"}</button>
              <button type="button" onClick={() => navigate(`/admin/stations/${id}`)} className="bg-gray-600 px-4 py-2 rounded text-white">Cancel</button>
            </div>
          </form>
        </div>
      </main>
